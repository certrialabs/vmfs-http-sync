...
module.exports = {
  start: function(address, port, sharedDir, destDir, logLevel) {
    winston.level = logLevel || 'warn';
    ...
    fs.remove(path.normalize(destDir), function(err) {
      if (err) {
        winston.error(err);
        return 1;
      }
      fs.copy(path.normalize(sharedDir), path.normalize(destDir), function(err) {
        if (err) {
          winston.error(err);
          return 1;
        }
