...
var removeOld = function(src, dst, callback) {
  ...
  var walker = constructWalker(dst, function(err) {
    if (err) {
      callback(err);
      return;
    }

    // TODO Clean this using promises
    nodes = nodes.sort().reverse();
    var fileCount = nodes.length;
    var removedFiles = 0;
    // In case we don't remove anything we need to call the callback, so we can
    // continue with coping new files.
    if (fileCount === 0) {
      callback();
    }
    var unlinkCallbackGenerator = function(node) {
      return function (err) {
        removedFiles++;
        if (err) { callback(err); return; }
        winston.debug('Removed ' + node);
        // Calls our callback only when all files are removed.
        if (fileCount === removedFiles) {
          callback();
        }
      };
    };
    ...
    for (var i = 0; i < fileCount; i++) {
      fs.remove(nodes[i], unlinkCallbackGenerator(nodes[i]));
    }
  });
  ...
  walker.on('node', function(root, stats, next) {
    ...
    fs.exists(dstNode, function(exists) {
      if (exists) {
